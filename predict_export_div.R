...
###############################################################################
# Join with export_div
###############################################################################

export_div <- fix_adm0(export_div)
trade_vars <- trade_vars %>%  fix_adm0() 
# Do country names match up?
setdiff(trade_vars$country,export_div$country)
setdiff(export_div$country,trade_vars$country)

j <- left_join(export_div,trade_vars,by=c('country','year')) %>% filter(year > 1959)
nrow(j)
nrow(na.omit(j))
colSums(is.na(j))
...
summary(my_lm)
# R^2 of 0.63; lots of significant variables
...
j2 %>%
  ...
  ggtitle('Export diversification')


mean(j2$nrmse) # 0.3795146
...
j2 %>%
  ...
  mutate(pred = (pred - mean(value))/sd(value),
         value = scale(value)) %>%
  write.csv('predict_export_div.csv',row.names=FALSE)
...
write.csv(future,'forecast_export_div.csv',row.names=FALSE)
glimpse(future)
